...
macro( add_pch pchTarget header module )
  ...
  add_custom_command(
    OUTPUT  "${outputHeader}.pch"
    DEPENDS ${pchTarget}_trigger
    COMMAND "${CMAKE_COMMAND}" -E copy_if_different "${inputHeader}" "${outputHeader}"
    COMMAND "${CMAKE_COMMAND}" -E remove -f "${outputHeader}.pch"
    COMMAND "${CMAKE_CXX_COMPILER}" ${flags} -o "${outputHeader}.pch" "${inputHeader}" )
  add_custom_target( ${pchTarget} DEPENDS "${outputHeader}.pch" )
...
macro( use_pch target pchTarget )
  ...
  set_source_files_properties( ${sources} PROPERTIES OBJECT_DEPENDS "${pchHeader}.pch" )
endmacro( use_pch )
